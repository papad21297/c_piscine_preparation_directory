...
/*
  Create a function that displays the memory
  area on screen.

  BIG BOY PROGRAM HERE, DIVIDE AND
  CONQUER APPROACH

  The display of this memory area should
  be split into three "columns" separated
  by a space :
  1)  The hexadecimal address of the first
      line's first character followed by
      a ':'.
  2)  The content in hexadecimal with a
      space each 2 characters and should
      be padded with spaces if needed.
  3)  The content in printable characters.

  CONSTRAINTS
  - If a character is non-printable, it'll
    be replaced by a dot.
  - Each line should handle sixteen characters.
    - Should I truncate if more?
  - If size equals to 0, nothing should be
    displayed
    - Empty string has size 1! --> \0 char
*/

// Wait, but... a byte is just an unsigned
// char...
...
int	main()
{
	// This string has size 1 and length 0
	//
	// If size equals to 0, nothing should
	// be displayed
	char	empty[] = "";
...
void	*ft_print_memory(void *addr, unsigned int size)
{
	...
	// displayed
	//
	// I assume they talk about an empty string
	// "", of size 1
	if (size == 1)
		return (addr);
...
// 3)  Print the string itself with dots
//     for NonPrintable
